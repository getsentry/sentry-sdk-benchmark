...
func UpdateHandler(db storage.DB) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for i := 0; i < queries; i++ {
			func(i int) {
				span := sentry.StartSpan(ctx, "db", func(s *sentry.Span) {
					s.Description = fmt.Sprintf("GetOneRandomWorld[i=%d]", i)
				})
				defer span.Finish()
				if err = db.GetOneRandomWorld(&worlds[i]); err != nil {
					log.Println(err)
		...
		if err = func() error {
			span := sentry.StartSpan(ctx, "db", func(s *sentry.Span) {
				s.Description = "UpdateWorlds"
			})
			defer span.Finish()
			return db.UpdateWorlds(worlds)
		}(); err != nil {
			log.Println(err)
			return
		}
		...
		w.Header().Set("Content-Type", "application/json")
		func() {
			span := sentry.StartSpan(ctx, "json.encode")
			defer span.Finish()
			json.NewEncoder(w).Encode(worlds)
		}()
